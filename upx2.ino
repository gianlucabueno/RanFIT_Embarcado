...
const char* password = "FacensLixo23";
...
float correnteGerada;
float potenciaEnvio;
int sendValue;
float leituraRealizada = 0;
...
void loop() {
  ...
  if (currentMillis - previousMillis >= 1000 || previousMillis == 0 ) {
    ...
    while(userID == valueMachine && sendValue == 0 && valueMachine != ""){
      leituraRealizada = analogRead(pinoSensor1);
      tensaoEntrada = (leituraRealizada * 5)/1024; 
      correnteGerada = tensaoEntrada / 10;
      potenciaGerada = tensaoEntrada * correnteGerada;
